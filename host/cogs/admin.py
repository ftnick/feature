...
def main(bot):
    ...
    @bot.command(name="_forceCPU", help="Starts a stress test for the bot with advanced system monitoring.")
    @roleLocked("developer", 1307619426757120041)
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def stress_test(ctx, seconds: int = 5, intensity: int = 1000000):
        ...
        def update_stats():
            ...
            while not stop_flag[0]:
                ...
                new_embed.add_field(name="CPU Usage", value=f"{cpu_usage}%", inline=True)
                new_embed.add_field(name="CPU Frequency", value=f"{cpu_freq} MHz", inline=True)
                new_embed.add_field(name="Memory Usage", value=f"{mem.percent}%", inline=True)
                new_embed.add_field(name="Disk Usage", value=f"{disk.percent}%", inline=True)
                new_embed.add_field(name="Elapsed Time", value=f"{elapsed}s", inline=True)
                ...
                asyncio.run_coroutine_threadsafe(message.edit(embed=new_embed), bot.loop)
        ...
        final_embed.add_field(name="Final CPU Usage", value=f"{final_cpu}%", inline=True)
        final_embed.add_field(name="Final Memory Usage", value=f"{final_mem}%", inline=True)
        final_embed.add_field(name="Final Disk Usage", value=f"{final_disk}%", inline=True)
        final_embed.add_field(name="Total Elapsed Time", value=f"{final_elapsed}s", inline=True)
